...
def joern_parse(file, outdir):
    ...
    os.system('/home/itachi/software/joern-cli/joern-parse $file --language c --out $out')
...
def joern_export(bin, outdir, repr):
    ...
    if repr == 'pdg':
        os.system('/home/itachi/software/joern-cli/joern-export $bin' + " --repr " + "pdg" + ' --out $out')  # cpg 改成 pdg
        try:
        ...
        os.system('/home/itachi/software/joern-cli/joern-export $bin' + " --repr " + "cpg14" + ' --out $out')  # cpg 改成 pdg
        try:
            cpg_list = os.listdir(out)
            for cpg in cpg_list:
                if cpg.startswith("0-cpg"):
        ...
        os.system('/home/itachi/software/joern-cli/joern-export $bin' + " --repr " + "cfg" + ' --out $out')  # cpg 改成 pdg
        try:
        ...
        os.system('/home/itachi/software/joern-cli/joern-export $bin' + " --repr " + "ddg" + ' --out $out')  # cpg 改成 pdg
        try:
            ddg_list = os.listdir(out)
            for ddg in ddg_list:
                if ddg.startswith("0-ddg"):
        ...
        if out[-4:] != 'json':
            out += '.json'
        joern_process = subprocess.Popen(["/home/itachi/software/joern-cli/joern"], stdin=subprocess.PIPE, stdout=subprocess.PIPE, shell=True, encoding='utf-8')
        import_cpg_cmd = f"importCpg(\"{bin}\")\r"
        script_path = f"/home/itachi/software/joern-cli/scripts/graph/graph-for-funcs.sc"
