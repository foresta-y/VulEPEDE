...
# node_labels0 = ["BLOCK", "CALL", "Comment", "Control_Structure", "File", "IDENTIFIER", "Field_Identifier", "Literal",
#                "Local", "Member", "MetaData", "METHOD", "MethodInst", "Method_Parameter_In", "Method_Parameter_Out",
#                "METHOD_RETURN", "Namespace", "NamespaceBlock", "PARAM", "RETURN", "Type", "TypeDecl", "Unknown"]
...
node_labels = ['UNKNOWN', 'METHOD', 'METHOD_RETURN', 'PARAM', 'CALL', 'RETURN', 'LITERAL', 
               'IDENTIFIER', 'FIELD_IDENTIFIER', 'equals', 'assignment', 'postDecrement','conditional', 'logicalAnd',  'multiplication', 
               'sizeOf', 'indirectFieldAccess', 'logicalNot', 'indirection', 
               'addressOf',  'assignmentPlus', 'addition', 'indirectIndexAccess', 
               'subtraction', 'assignmentAnd','lessThan', 'greaterThan', 'minus', 'not', 'notEquals', 'and', 
               'shiftLeft', 'postIncrement', 'fieldAccess', 'logicalOr', 'lessEqualsThan', 'cast', 
               'division', 'modulo', 'assignmentMinus', 'greaterEqualsThan', 'or', 'arithmeticShiftRight', 
               'preDecrement', 'assignmentShiftLeft', 'assignmentOr', 'assignmentArithmeticShiftRight', 'preIncrement', 
               'assignmentMultiplication', 'assignmentDivision', 'assignmentXor', 'plus', 'asm', 'float']
...
class Node:
    def __init__(self, node, attrs, edges):
        self.in_edges = []
        self.out_edges = []
        self.in_nodes = []
        self.out_nodes = []
        self.set_properties(node, attrs)
        self.get_edges(edges)
        
    def set_properties(self, node, attrs):
        self.id = node
        self.line_info = attrs["label"].replace("\\n", "")[2:-2]
        self.node_type = self.line_info.split(",")[0]
        # self.code = self.line_info.split(",")[1]
        # 代码截取第一个逗号后的部分
        self.code = self.line_info[self.line_info.index(",") + 1:]
        if ("<operator>" in self.node_type) or ("<operators>" in self.node_type):
            self.node_type = self.node_type.split(".")[1]
        
        if self.node_type in list_sensitive_funcname or 'fun' in self.node_type:
            self.node_type = "CALL"
    ...
    def get_edges(self, edges):
        for edge in edges:
            if edge.node_in == self.id:
                self.out_edges.append(edge)
                self.out_nodes.append(edge.node_out)
            if edge.node_out == self.id:
                self.in_edges.append(edge)
                self.in_nodes.append(edge.node_in)
        
    def _set_label(self):
        self.label = node_labels.get(self.node_type)
        if self.label is None:
            self.label = node_labels.get("UNKNOWN")
            
    def __str__(self):
        return f"Node {self.id} ({self.node_type}): {self.code}"
